...
//-----------------------------------------------------------------------------------------
//----------------------------------score update part-------------------------------------
        let dailyApiEvents0 = await db.collection('DailyApiEvents').find().toArray()
        ...
        if(index_score1 % 3 == 1){
            ...
            for(i = 0 ; i < eventResult.length ; i++){
                if(eventResult[i].Teams2[0] && eventResult[i].Teams1[0]){
                    if(eventResult[i].Teams2[0].url_role.includes('https') && eventResult[i].Teams2[0].url_market.includes('https') && eventResult[i].Teams1[0].url_role.includes('https') && eventResult[i].Teams1[0].url_market.includes('https') && (eventResult[i].Teams2[0].update_role_date < next_date || eventResult[i].Teams2[0].update_market_date < next_date || eventResult[i].Teams1[0].update_role_date < next_date || eventResult[i].Teams1[0].update_market_date < next_date)){
...
//--------------------------------
            ...
            // let next_date1 = start_date1.setDate(start_date1.getDate() - 1);
            // let end_date1 = start_date1.setDate(start_date1.getDate() + 1);
            ...
            // let events = await db.collection('eventsTotal').find({"openDate": {$gte: next_date1, $lt: end_date1}}).sort({"openDate": 1}).toArray()
            console.log(next_date1, end_date1)
            let events = await db.collection('eventsTotal').aggregate([
                { $match: {"openDate": {$gte: next_date1, $lt: end_date1}}},
                { $lookup: { from: "competitionsTotal", localField: "topLevelEventId", foreignField: "eventId", as: "competitions" }}
            ]).sort({"openDate": 1}).toArray();
            console.log('evenLength==>', events.length)
            let service = new betfair();
            ...
            for(i = 0 ; i < data.length ; i++){
                if(data[i]){
                    for(j = 0; j < events.length ; j++){
                        ...
                        // console.log(data[i].localTeam.data.name, ' ', home, '', events[j].eventId)
                        // console.log(data[i].visitorTeam.data.name, ' ', away)
                        if(data[i].localTeam.data.name == home && data[i].visitorTeam.data.name == away){
                            k++
                            // console.log(data[i].localTeam.data.name, ' v ', data[i].visitorTeam.data.name, ' ', events[j].eventId)
                            let id =  data[i].id
                            let query = {"_id": data[i].id}
                            let dup = await db.collection('liveStatsTotal').findOne({'_id': data[i].id})
                            let dup1 = await db.collection('liveStats').findOne({'_id': data[i].id})
                            if(!dup){
                            ...
                            if(!dup){
                                if(data[i].time.status == "LIVE") {
                                    await db.collection('liveStats').updateOne(query,
                                        ...
                                        {upsert: true},
                                        function (err, item) { if (err){ return }})
                                }
                            }
...
//---------------------------------------------live New Part------------------------------------------------
                                let time_check1 = await db.collection('liveStatsTotal').find({"_id": data[i].id }).toArray()
                                let timeLength1 = 0
                                if(time_check1.length > 0){
                                    timeLength1 = time_check[0].time.length
                                }

                                let time1 = 0
                                let extra_time1 = 0
                                if(timeLength1 > 0){
                                    time1 = time_check1[0].time[timeLength - 1].minute
                                    extra_time1 = time_check1[0].time[timeLength - 1].injury_time
                                }
                                if(extra_time1 === null){
                                    extra_time1 = 0
                                }
...
//---------------------------------------------------------------------------------------------------------
                            ...
                            //     let updateArray = await db.collection('liveStatsTotal').findOne({"_id": id})
            ...
            for(i = 0 ; i < eventResult.length ; i++){
                if(eventResult[i].Teams2[0] && eventResult[i].Teams1[0]){
                    if(eventResult[i].Teams2[0].titularity_update_date < next_date || eventResult[i].Teams1[0].titularity_update_date < next_date){
                    ...
                    await db.collection('Team_T_A_map').updateOne(query,
                        {
                            $set: {
                                ...
                                'titularity_update_date': next_date
                            }
                        },
                        {upsert: true},
                        function (err, item) {
        ...
        //     next_date = next_date.substring(0,11)+'00:00:00'+ next_date.substring(19,24)
        //     end_date = end_date.substring(0,11)+'04:00:00'+ end_date.substring(19,24)
        //     let openDate = next_date.substring(0,10)
        //     let fixtures = await db.collection('fixtures').find({"date": openDate}).toArray()
        //     let events = await db.collection('eventsTotal').find({"openDate": {'$gte': next_date, "$lt": end_date}}).toArray()
        //
        //     let fixtureArray = []
        //     let fixtureIndex = 0
        //     for(i = 0 ; i < events.length ; i++){
        //         let str =  events[i].name
        //         let home = str.split(' v ')[0].trim()
        //         let away = str.split(' v ')[1].trim()
        //         for(j = 0 ; j < fixtures.length ; j++){
        //             let home1 = fixtures[j].home
        //             let away1 = fixtures[j].away
        //             if(home == home1 && away == away1){
        //                 fixtureIndex++
        //                 fixtureArray[fixtureIndex - 1] = fixtures[j]._id
        //             }
        //         }
        //     }
        //
        //     if(indexf > fixtureArray.length){
        //         indexf = 1
        //     }
        //
        ...
        //         if(i == indexf - 1){
        //             let service = new betfair();
        //             let data = await service.getLiveStatistics(fixtureArray[indexf - 1].toString())
        //             console.log('data==>', data)
        //             console.log('fixtureArray==>', fixtureArray)
        //             console.log('fixtureId===>',fixtureArray[i])
        //         }
        //     }
        // }
        // else if(index_score1 % 7 == 7){
        //     let start_date = new Date();
        //     let next_date = start_date.setDate(start_date.getDate() + 0);
        //     next_date = new Date(next_date).toISOString()
        //     let openDate = next_date.substring(0,10)
        //
        //     let service = new betfair();
        //     let fixtures = await service.getFixtureIds(openDate)
        //     let teamData = []
        //     for(i = 0 ; i < fixtures.response.length ; i++){
        //         teamData.push({'_id':fixtures.response[i].fixture.id,'date':fixtures.parameters.date, 'timestamp':fixtures.response[i].fixture.timestamp,'leagueId': fixtures.response[i].league.id,'leagueName': fixtures.response[i].league.name, 'country':fixtures.response[i].league.country, 'home':fixtures.response[i].teams.home.name, 'away':fixtures.response[i].teams.away.name, 'status':fixtures.response[i].fixture.status})
        //     }
        //     await db.collection('fixtures').deleteMany({'_id': {$ne: "1"}})
        //     db.collection('fixtures').insertMany(teamData)
        // }
    });
    taskScore.start()
    ...
    cold = schedule('0 */5 * * * *', async () => {
        ...
        if(coldIndex % 4 == 0){
            ...
            let rounds = await db.collection('DailyApiEvents').find({"time.starting_at.date": {$gte: next_date5, $lte: next_date4}, "round_id": {$ne: null}}).toArray()
    ...
    app.post('/getLiveStats',async (req,res)=>{
        try {
            ...
            console.log('livestatusLength==>', end_date2)
            ...
            let eventResult2 = await db.collection('DailyApiEvents').aggregate([
                { $match: {"time.starting_at.date": {$gte: next_date2} }},
                { $lookup: { from: "competitionsTotal", localField: "competitionId", foreignField: "eventId", as: "competitions" } }
            ]).sort({"openDate": 1}).toArray();
            res.status(200).send({
                data: [eventResult, eventResult1, eventResult2],
                message: 'success'
            })
        }catch (e) {
    ...
    app.post('/savePredictionData',async (req,res)=>{
        ...
        if(event.length > 0){
            ...
            id = openDate.substring(11, 16) + " >> " + eventName + " " + marketType
